...
function tabAndNavClick() {
    ...
    $('.left-nav').on('click','.nav-slide',function(event) {
        if($('.left-nav').css('width')=='220px'){
            leftNavShort();
        }else{
            leftNavLong();
    ...
    //allen --- 鼠标离开左侧菜单收起
    // var idSideNav = null;
    // $('.left-nav #side-nav').on('mouseleave', function(event) {
    //     event.stopPropagation();//阻止事件冒泡
    //     idSideNav = setTimeout(function () {
    //         leftNavShort();
    //     },600)
    // });
    // $('.left-nav #side-nav').on('mouseenter', function(event) {
    //     event.stopPropagation();//阻止事件冒泡
    //     idSideNav && clearTimeout(idSideNav);
    // });
...
//allen --- logo那行的适应
function logoHandleShort (){
    $('.container-right').animate({left: '60px'}, 100);
    $('.logo').animate({width: '60px'}, 100);
    $('.logo').find('.logo-small').show();
    $('.logo').find('.logo-big').hide();
    $('.nav-slide>div:last-of-type').addClass('nav-slide-open');
}
function logoHandleLong (){
    $('.container-right').animate({left: '220px'}, 100);
    $('.logo').animate({width: '220px'}, 100);
    $('.logo').find('.logo-small').hide();
    $('.logo').find('.logo-big').show();
    $('.nav-slide>div:last-of-type').removeClass('nav-slide-open');
}
// if(typeof layui['layui.all'] == 'undefined'){
